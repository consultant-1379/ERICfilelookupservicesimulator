 ...
 *******************************************************************************
 *----------------------------------------------------------------------------*/
package com.ericsson.oss.services.fls.processor.pmic;
...
/**
 * PMIC Processor class read pmic resources, validate, persist to postgres and delete it from NFS.
 *
 * @author ehimnay
 */
@ApplicationScoped
@ClientTypeAnnotation(clientType = RequestType.PMIC)
public class PMICProcessor extends AbstractClientProcessor {
    ...
    public void createPMRopInfo(final List<String> resourcesList)
    {
        ...
        if(!pmRopInfoList.isEmpty()) {
            ...
            pmRopInfoDao.createPmRopInfo(pmRopInfoList);
        }
    }
}
